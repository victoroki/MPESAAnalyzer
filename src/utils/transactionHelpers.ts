...
export const getCurrentMonthStats = (transactions: Transaction[]): MonthlyStats => {
  const now = new Date();
  return calculateMonthlyStats(transactions, now.getFullYear(), now.getMonth());
};

export const getLastNMonthsStats = (transactions: Transaction[], n: number): MonthlyStats[] => {
